...
def graph_coloring(graph):
    """
    DSatur graph coloring with enhanced tie-breaking.

    DSatur prioritizes vertices with highest saturation degree (most distinct
    neighbor colors), with tie-breaking using uncolored degree, total degree,
    and vertex ID.
    ...
    # Start with highest degree vertex to establish good initial coloring
    if uncolored:
        first_vertex = max(uncolored, key=lambda v: (degrees[v], -v))
    ...
    while uncolored:
        ...
        best_vertex = -1
        best_saturation = -1
        best_uncolored_degree = -1
        best_degree = -1
        best_neighbor_colors = set()
        ...
        for vertex in uncolored:
            ...
            # Enhanced tie-breaking: saturation first, uncolored degree, total degree, then higher vertex ID
            # Higher vertex ID can help with certain graph structures
            if (saturation > best_saturation or 
                (saturation == best_saturation and uncolored_neighbors > best_uncolored_degree) or
                (saturation == best_saturation and uncolored_neighbors == best_uncolored_degree and degrees[vertex] > best_degree) or
                (saturation == best_saturation and uncolored_neighbors == best_uncolored_degree and degrees[vertex] == best_degree and vertex > best_vertex)):
                best_vertex = vertex
                best_saturation = saturation
                best_uncolored_degree = uncolored_neighbors
                best_degree = degrees[vertex]
    ...
    # Enhanced post-processing: try to compact colors starting from highest
    # Process top 2 colors to balance quality and performance
    if coloring:
        max_color = max(coloring.values())
        if max_color > 0:
            # Try to reduce usage of top 2 colors
            for target_color in range(max_color, max(0, max_color - 2), -1):
                vertices_with_color = [v for v, c in coloring.items() if c == target_color]
                
                # Try to recolor each to a lower color
