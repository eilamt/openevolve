...
def graph_coloring(graph):
    ...
    Dynamically selects vertices with highest saturation degree
    (most different colors among neighbors) for better coloring.
    ...
    sat_degree = [0] * graph.num_vertices
    ...
    while uncolored:
        ...
        for v in uncolored:
            if sat_degree[v] > max_sat or (sat_degree[v] == max_sat and graph.get_degree(v) > max_deg):
                max_sat = sat_degree[v]
                max_deg = graph.get_degree(v)
                next_vertex = v
        ...
        for neighbor in graph.get_neighbors(next_vertex):
            if neighbor in uncolored:
                ...
                for nn in graph.get_neighbors(neighbor):
                    if nn in coloring:
                        neighbor_neighbor_colors.add(coloring[nn])
                sat_degree[neighbor] = len(neighbor_neighbor_colors)
