...
def graph_coloring(graph):
    ...
    while uncolored_vertices:
        # Calculate saturation degree and degree for uncolored vertices
        vertex_scores = []
        for vertex in uncolored_vertices:
            ...
            for neighbor in graph.get_neighbors(vertex):
                if neighbor in coloring:
                    ...
                    # Count degree in the uncolored subgraph
                    degree += 1
            saturation_degree = len(neighbor_colors)
            vertex_scores.append((saturation_degree, degree, vertex))

        # Sort vertices: primarily by saturation degree (descending),
        # then by degree (descending), then by vertex index (ascending)
        vertex_scores.sort(key=lambda x: (-x[0], -x[1], x[2]))

        # Select the vertex with the highest score
        best_vertex = vertex_scores[0][2]

        # Find colors used by neighbors of the selected vertex
        neighbor_colors = set()
        ...
        # Remove the vertex from the uncolored set
